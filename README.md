...
##### 비교 연산자
삼항연산자를 기본으로하며 상황에따라서 다.르게 써도 됩니다.
...
상수와 은닉은 섞어 사용하지 않습니다. 상수는 값이 변할 수 없다.는 의미를 가지고 있고 은닉은 값이 숨겨져 할당된 값을 수정할 수 없는 위치에 있는것입니다.
````
    //Bad
...
#### 파일
아래 순서대로 이름을 .단위로 조합하여 파일이름을 작명합니다.
...
다.수의 호출을 피하고 묶을 수 있는 메소드는 묶어 사용합니다.
````
    //example1
