# 자바스크립트 가이드
이 문서는 기준으로 일관성, 가독성, 유지보수 용이성을 위한 참고자료입니다.

## 기본

### strict 모드
js파일 또는 스크립트 태그 맨 윗줄에 'use strict'를 반드시 적습니다.
...
변수선언을 제외한 중괄호({})에서 중괄호 마침(})뒤에 세미콜론을 넣지 않습니다.
````
    //Bad
    function helloWorld() {
	//statement
...
직접 배열에 항목을 대입하지않고 push메소드를 사용합니다.
````
    //Bad
    var helloWorld = [];
...
## 함수
['http://usejsdoc.org/'](http://usejsdoc.org/){: target="_blank" }에 맞춰 작성합니다.
